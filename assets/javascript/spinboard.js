//spinboard object literal, holds all functions to handle what is required
var spinboard = {
		...
		switch( page ) {
			case 'main':
				...
				for( var i in this.curr_options ) { //loop through options, create copy of 'option' template, and fill out. add to bottom of options container
					(function() { //run self executing function to provide better scoping for variables in closures
						...
						option.find('.e-wppoo-action.t-delete').on('click.spinboard',function(e) { //add an action to run when someone clicks 'delete'
							...
							option.fadeOut('fast',function() { //fade out option, after complete, remove from curr_options array
								that.curr_options = that.curr_options.filter(function( item,key ) {
									return ( parseInt( i ) !== key ); //filter out anything that matches the deleted items index
								});
								option.remove(); //remove item from dom
								that.render_page('main'); //rerender page - used to make 'none' option show if the last item is removed instead of refactoring code to handle this (not the best way)
							});
